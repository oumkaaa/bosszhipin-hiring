            ...
            for candidate in allocated:
                try:
                    ...
                    if screen_result['screen_result'] == 'PASS' and score >= 75:
                        logger.info(f"  [{source}] {name} - Qualified, sending greeting")
                        ...
                        if first_msg:
                            ...
                            if send_result.get('code') == 0 or send_result.get('dry_run'):
                                ...
                                logger.warning(f"  [{source}] {name} - Failed to send: {send_result.get('message')}")
                                candidate['status'] = 'FAILED'
                                candidate['exclude_reason'] = f"Send failed: {send_result.get('message')}"
                        else:
                            logger.warning(f"  [{source}] {name} - No greeting message configured")
                            candidate['status'] = 'FAILED'
                            candidate['exclude_reason'] = 'No greeting message'
                    else:
