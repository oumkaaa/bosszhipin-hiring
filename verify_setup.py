...
def check_boss_agent_cli():
    """Check boss-agent-cli installation and version."""
    print("\n[2/7] Checking boss-agent-cli...")
    try:
        import importlib.metadata
        ver = importlib.metadata.version("boss-agent-cli")
        print(f"  [OK] boss-agent-cli {ver}")

        # Check version >= 1.13.1
        from packaging import version
        if version.parse(ver) >= version.parse("1.13.1"):
            return True
        else:
            print(f"  [WARN] boss-agent-cli {ver} < 1.13.1 required")
            return False
    except Exception as e:
        print(f"  [FAIL] Could not check boss-agent-cli: {e}")
        return False
...
def check_no_pycache():
    """Check that __pycache__ is not in git-tracked files.

    Note: This checks git status, not the working directory,
    to avoid self-generating __pycache__ during the check.
    """
    print("\n[6/7] Checking for __pycache__ in git...")
    try:
        ...
        if result.stdout.strip():
            print(f"  [WARN] __pycache__ is tracked in git")
            return False
        else:
            print("  [OK] No __pycache__ in git")
            return True
    except Exception as e:
        print(f"  [SKIP] Could not check git: {e}")
        return True
